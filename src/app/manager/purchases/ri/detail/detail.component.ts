...
export class DetailComponent {
  ...
  onRouter(value: string) {
    ...
    this.poService.findPoByPoId(this.token.getUser().uniqueKey, value).subscribe({
      next: (data) => {
        this.router.navigate(['/manager/purchases/po/detail', data.id]);
      },
    });

  }
  retrievlRiPaymentDetail() {
    this.loadingBar.show();

    this.route.params.subscribe((params) => {
      this.riPaymentService.getAllById(this.token.getUser().uniqueKey, this.token.getBranchId(), params['id']).subscribe(
        (data) => {
          this.getRiPayment = data;
          console.log(this.getRiPayment)
  ...
  onPrint(id: string) {
    this.router.navigate(['/manager/purchases/ri/print', id]);
  }
  ...
  onUnApporved(id: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.riService.unapproved(this.token.getUser().uniqueKey, id).subscribe(
          reponse => {
            if (reponse.message == "Successfully!") {
              this.loadingBar.hide();
              Swal.fire({
          ...
          error => {
            ...
            if (error.status == 500) {
              Swal.fire({
                ...
                title: 'Oops...',
                confirmButtonColor: '#07cdae',
                text: 'เมื่ออนุมัติแล้วไม่สามารถยกเลิกได้',
              })
            }
  ...
  payment() {
    ...
    this.riPayment = new RiPayment();

    this.riPayment.total = this.getOrderDetail.totalAll;
    this.riPayment.description = this.f['description'].value;
    this.riPayment.paymentType = this.f['type'].value;
    this.riPayment.datePayment = this.f['date'].value.day + "/" + this.f['date'].value.month + "/" + this.f['date'].value.year;
    this.riPayment.time = this.f['time'].value;
    this.riPayment.dateTime = this.f['date'].value;
    this.riPayment.bank = this.f['bank'].value;
    this.riPayment.uniqueKey = this.token.getUser().uniqueKey;
    this.riPayment.branchId = this.token.getBranchId();
    this.riPayment.riId = this.getOrderDetail.id;

    Swal.fire({
      title: 'เพิ่มการชำระเงิน?',
      text: "คุณต้องการเพิ่มการชำระเงิน",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#07cdae',
      cancelButtonColor: '#fe7096',
      ...
      if (result.isConfirmed) {
        this.loadingBar.show();
        this.riPaymentService.create(this.riPayment).subscribe(
          (reponse) => {
            if (reponse.message == 'Successfully!') {
              this.loadingBar.hide();
  ...
  onUpdatePayment(id: string) {


    this.loadingBar.show();

    this.riPaymentService.findById(this.token.getUser().uniqueKey, this.token.getBranchId(), id).subscribe(
      data => {
        this.getRiPaymentById = data;

        this.paymentUpdateForm.patchValue({ date: this.getRiPaymentById.dateTime });
        this.paymentUpdateForm.patchValue({ type: this.getRiPaymentById.paymentType });
        this.paymentUpdateForm.patchValue({ description: this.getRiPaymentById.description });
        this.paymentUpdateForm.patchValue({ time: this.getRiPaymentById.time });
        this.paymentUpdateForm.patchValue({ bank: this.getRiPaymentById.bank });
        this.loadingBar.hide();
      },
      error => {
        this.loadingBar.hide();
  ...
  paymentUpdate() {
    ...
    this.riPayment.datePayment = this.fUpdate['date'].value.day + "/" + this.fUpdate['date'].value.month + "/" + this.fUpdate['date'].value.year;
    this.riPayment.description = this.fUpdate['description'].value;
    this.riPayment.paymentType = this.fUpdate['type'].value;
    this.riPayment.time = this.fUpdate['time'].value;
    this.riPayment.bank = this.fUpdate['bank'].value;
    this.riPayment.dateTime = this.fUpdate['date'].value;

    Swal.fire({
      title: 'แก้ไขการชำระเงิน?',
      text: "คุณต้องการแก้ไขการชำระเงิน",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#07cdae',
      cancelButtonColor: '#fe7096',
      ...
      if (result.isConfirmed) {
        ...
        this.riPaymentService.update(this.getRiPaymentById.id, this.riPayment).subscribe(
          (reponse) => {
            if (reponse.message == 'Successfully!') {
              this.loadingBar.hide();

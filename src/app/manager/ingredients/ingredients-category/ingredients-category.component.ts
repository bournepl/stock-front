...
@Component({
  ...
  styleUrls: ['./ingredients-category.component.scss']
})
export class IngredientsCategoryComponent {
  ...
  fileNameExport = 'IngredientCategorySheet.xlsx';
  ...
  retrieveCategory() {
    ...
    console.log(this.token.getBranchId());
  ...
  create() {
    ...
    this.loadingBar.show();
    this.category = new IngredientsCategory();

    this.category.ingredientCategoryName = this.f['categoryName'].value;
    this.category.description = this.f['description'].value;
    this.category.ingredientCategoryId = this.f['categoryId'].value;
      ...
      .subscribe({
        next: (res) => {
          ...
          if (res.message == "Successfully!") {
            ...
            Swal.fire({
              icon: 'success',
              title: 'เพิ่มหมวดหมู่วัตถุดิบสำเร็จ',
  ...
  onDetail(id: string) {
    ...
    this.categoryService.getById(this.token.getUser().uniqueKey, id).subscribe(
      data => {
        ...
        this.categoryUpdateForm.patchValue({ categoryName: this.getCategoryById.ingredientCategoryName });
        this.categoryUpdateForm.patchValue({ categoryId: this.getCategoryById.ingredientCategoryId });
  ...
  update() {
    ...
    this.category.ingredientCategoryName = this.fUpdate['categoryName'].value;
    this.category.ingredientCategoryId = this.fUpdate['categoryId'].value;
  ...
  onDelete(id: any) {
    ...
    Swal.fire({
      ...
      text: "คุณต้องกาลบหมวดหมู่วัตถุดิบ",
      icon: "question",
          ...
          .subscribe({
            next: (data) => {
              ...
              if (data.message == "Successfully!") {
                ...
                Swal.fire({
                  icon: 'success',
                  title: 'ลบหมวดหมู่วัตถุดิบสำเร็จ',
                  showConfirmButton: false,
                  timer: 1500
                }).then(() => {
                  this.retrieveCategory();
  ...
  upload(): void {
    ...
    if (this.selectedFiles) {
      ...
      if (file) {
        ...
        this.excelService.uploadIngredientsCategory(this.token.getUser().uniqueKey, this.token.getBranchId(), this.currentFile).subscribe({
          next: (event: any) => {

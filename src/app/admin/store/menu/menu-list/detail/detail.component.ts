...
export class DetailComponent {
  ...
  selectedFilesPackage?: FileList;
  currentFilePackage?: File;
  progressPackage = 0;
  messagePackage = '';
  fileNamePackage: string;
  @ViewChild('staticModalImportPackage', { static: false }) staticModalImportPackage: ModalDirective;
  ...
  retrieveMenu() {
    ...
    this.route.params.subscribe(params => {
      this.manuService.getById(this.token.getUser().admin, params['id']).subscribe(data => {
        this.getMenu = data;

        this.menuForm.patchValue({ category: this.getMenu.menuCategory.id });
        this.menuForm.patchValue({ menuName: this.getMenu.menuName });
        this.menuForm.patchValue({ description: this.getMenu.description });

        this.getCategoryById = this.getMenu.menuCategory;

        if (this.getMenu.ingredients == null) {
          this.cartIngredientsList = [];
        } else {
          this.cartIngredientsList = this.getMenu.ingredients;
        }

        if (this.getMenu.packageIngredients == null) {
          this.cartIngredientsListPackage = []
        } else {
          this.cartIngredientsListPackage = this.getMenu.packageIngredients;
        }
  ...
  create() {
    ...
    this.manuService.update(this.token.getUser().uniqueKey, this.getMenu.id, this.menu).subscribe({
      next: (res) => {
        ...
        if (res.message == 'Successfully!') {
          ...
          Swal.fire({
            ...
            this.router.navigate(['/admin/store/menu/menu-list/list']);
          });
        }
  ...
  upload(): void {
    ...
    if (this.selectedFiles) {
      ...
      if (file) {
        ...
        this.excelService.uploadImportIng(this.token.getUser().uniqueKey, this.token.getBranchId(), this.getMenu.id, this.currentFile).subscribe({
          next: (event: any) => {
  ...
  */
  ...
  showChildModalImportPackage(): void {
    this.staticModalImportPackage.show();
  }
  hideChildModalImportPackage(): void {

    this.staticModalImportPackage.hide();
  }


  selectFilePackage(event: any): void {
    this.selectedFilesPackage = event.target.files;
    this.fileNamePackage = this.selectedFilesPackage!.item(0)!.name;
  }


  uploadPackage(): void {
    this.progressPackage = 0;

    if (this.selectedFilesPackage) {
      const file: File | null = this.selectedFilesPackage.item(0);

      if (file) {

        this.currentFilePackage = file;

        this.excelService.uploadImportIngPackage(this.token.getUser().uniqueKey, this.token.getBranchId(), this.getMenu.id, this.currentFilePackage).subscribe({
          next: (event: any) => {
            if (event.type === HttpEventType.UploadProgress) {
              this.progressPackage = Math.round(100 * event.loaded / event.total);
            } else if (event instanceof HttpResponse) {
              if (event.body.message == "Successfully!") {
                this.loadingBar.hide();
                Swal.fire({
          ...
          error: (err: any) => {
            ...
            Swal.fire({
              ...
              title: 'Oops...',
              confirmButtonColor: '#07cdae',
              text: this.messagePackage,
            })


            this.currentFilePackage = undefined;
          }
        });
      }

      this.selectedFilesPackage = undefined;
    }
  }

...
export class CartPoService {
  ...
  loadCart(): void {
    this.cartItemList = JSON.parse(sessionStorage.getItem("cart_po_items")!) ?? [];
  }
  ...
  addProductToCart(product: Ingredients) {
    ...
    sessionStorage.setItem("cart_po_items", JSON.stringify(this.cartItemList));
  ...
  removeAllCart() {
    this.cartItemList = [];
    sessionStorage.removeItem("cart_po_items")
  }
